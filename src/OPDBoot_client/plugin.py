...
class OPDBootImageChoose(Screen):
    ...
    def __init__(self, session):
        ...
        self.list = []
        self.setTitle('OPDBoot %s - Client_Menu' % PLUGINVERSION)
...
def main(session, **kwargs):
    f = open(
        '/usr/lib/enigma2/python/Plugins/Extensions/OPDBoot_client/.opdboot_location', 'r')
    mypath = f.readline().strip()
    f.close()
...
def Plugins(**kwargs):
    return [PluginDescriptor(name='OPDBoot', description=' OPD MultiBoot_Client', where=PluginDescriptor.WHERE_MENU, fnc=menu), PluginDescriptor(name='OPDBoot', description=_('E2 Light Multiboot'), icon='OPDBootFHD.png', where=PluginDescriptor.WHERE_PLUGINMENU, fnc=main)]
