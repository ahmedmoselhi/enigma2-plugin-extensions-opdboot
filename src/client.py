...
class HTTPPageGetter(http.HTTPClient):
    ...
    def handleEndHeaders(self):
        ...
        m = getattr(self, 'handleStatus_' +
                    nativeString(self.status), self.handleStatusDefault)
        m()

    def handleStatus_200(self):
        pass

    def handleStatus_201(self): return self.handleStatus_200()
    def handleStatus_202(self): return self.handleStatus_200()
    ...
    def handleStatus_301(self):
        ...
        if self.followRedirect:
            ...
            if self.factory.scheme == 'https':
                ...
                reactor.connectSSL(nativeString(
                    self.factory.host), self.factory.port, self.factory, contextFactory)
            else:
                reactor.connectTCP(nativeString(
                    self.factory.host), self.factory.port, self.factory)
        else:
            self.handleStatusDefault()
    ...
    def timeout(self):
        self.quietLoss = True
        self.transport.loseConnection()
        self.factory.noPage(defer.TimeoutError(
            'Getting %s took longer than %s seconds.' % (self.factory.url, self.factory.timeout)))
    ...
    def __init__(self, url, method='GET', postdata=None, headers=None, agent='Twisted PageGetter', timeout=0, cookies=None, followRedirect=True, redirectLimit=20, afterFoundGet=False):
        ...
        if postdata is not None:
            self.headers.setdefault(
                'Content-Length', intToBytes(len(postdata)))
...
class HTTPDownloader(HTTPClientFactory):
    ...
    def __init__(self, url, fileOrName, method='GET', postdata=None, headers=None, agent='Twisted client', supportPartial=0, timeout=0, cookies=None, followRedirect=1, redirectLimit=20, afterFoundGet=False):
        ...
        HTTPClientFactory.__init__(self, url, method=method, postdata=postdata, headers=headers, agent=agent, timeout=timeout,
                                   cookies=cookies, followRedirect=followRedirect, redirectLimit=redirectLimit, afterFoundGet=afterFoundGet)
        return
...
def _makeGetterFactory(url, factoryFactory, contextFactory=None, *args, **kwargs):
    ...
    if uri.scheme == 'https':
        ...
        if contextFactory is None:
            contextFactory = ssl.ClientContextFactory()
        reactor.connectSSL(nativeString(uri.host), uri.port,
...
class _RetryingHTTP11ClientProtocol(object):
    ...
    def _shouldRetry(self, method, exception, bodyProducer):
        ...
        if method not in ('GET', 'HEAD', 'OPTIONS', 'DELETE', 'TRACE'):
            return False
        elif not isinstance(exception, (RequestNotSent, RequestTransmissionFailed, ResponseNeverReceived)):
            return False
        ...
        while connections:
            ...
            if connection.state == 'QUIESCENT':
                if self.retryAutomatically:
                    def newConnection(): return self._newConnection(key, endpoint)
...
class _AgentBase(object):
    ...
    def _requestWithEndpoint(self, key, endpoint, method, parsedURI, headers, bodyProducer, requestPath):
        ...
        if not headers.hasHeader('host'):
            ...
            headers.addRawHeader('host', self._computeHostValue(
...
class Agent(_AgentBase):
    ...
    def _getEndpoint(self, scheme, host, port):
        ...
        if scheme == 'http':
            ...
            return SSL4ClientEndpoint(self._reactor, host, port, self._wrapContextFactory(host, port), **kwargs)
        else:
...
class ProxyAgent(_AgentBase):
    ...
    def request(self, method, uri, headers=None, bodyProducer=None):
        ...
        return self._requestWithEndpoint(key, self._proxyEndpoint, method, _URI.fromBytes(uri), headers, bodyProducer, uri)
...
class RedirectAgent(object):
    ...
    @since: 11.1
    """
    _redirectResponses = [http.MOVED_PERMANENTLY,
                          http.FOUND, http.TEMPORARY_REDIRECT]
    ...
    def _handleRedirect(self, response, method, uri, headers, redirectCount):
        ...
        if redirectCount >= self._redirectLimit:
            err = error.InfiniteRedirection(
                response.code, 'Infinite redirection detected', location=uri)
...
class _ReadBodyProtocol(protocol.Protocol):
    ...
    def connectionLost(self, reason):
        ...
        if reason.check(ResponseDone):
            ...
            self.deferred.errback(PartialDownloadError(
                self.status, self.message, ''.join(self.dataBuffer)))
        else:
            self.deferred.errback(reason)
