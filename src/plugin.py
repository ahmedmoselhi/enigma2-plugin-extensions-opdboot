...
choicelist = [('0', _("Audio Selection")), ('1', _("Default (Timeshift)")),
              ('2', _("Toggle Pillarbox <> Pan&Scan")), ('3', _("Teletext"))]
config.plugins.OPDBoot_yellowkey = ConfigSubsection()
config.plugins.OPDBoot_yellowkey.list = ConfigSelection(
    default='1', choices=choicelist)
config.plugins.OPDBoot_yellowkey.listLong = ConfigSelection(
    default='1', choices=choicelist)
config.plugins.OPDBoot_yellowkey.list = ConfigSelection(
...
class OPDBootInstallation(Screen):
    ...
    def __init__(self, session):
        ...
        self['label1'] = Label(
            _('Welcome to OPDBoot %s MultiBoot Plugin installation.') % PLUGINVERSION)
        self['label2'] = Label(
    ...
    def updateList(self):
        ...
        if fileExists('/proc/mounts'):
            ...
            self['label2'].setText(
                _('Sorry it seems that there are not Linux formatted devices mounted on your STB. To install OPDBoot you need a Linux formatted part1 device. Click on the blue button to open OPD Devices Panel'))
            fileExists('/proc/mounts')
        if myusb:
    ...
    def devpanel(self):
        try:
            ...
            self.session.open(MessageBox, _(
                'You are not running OPD Image. You must mount devices Your self.'), MessageBox.TYPE_INFO)
    ...
    def install(self):
        ...
        if check is False:
            ...
            if self.checkReadWriteDir(self.mysel):
                message = _(
                    'Do You really want to install OPDBoot in:\n ') + self.mysel + '?'
                ybox = self.session.openWithCallback(
                    self.install2, MessageBox, message, MessageBox.TYPE_YESNO)
    ...
    def install2(self, yesno):
        ...
        config.OPDBootmanager.bootmanagertimeout = ConfigSelection([('5', _("5 seconds")), ('10', _(
            "10 seconds")), ('15', _("15 seconds")), ('20', _("20 seconds")), ('30', _("30 seconds"))], default='5')
        if getMachineBuild() in ("u5", "u51", "u52", "u53", "u54", "u55", "u56", "u41", "u5pvr", "sf8008", "sf8008m"):
            ...
            message = _(
                'Do you want to use Bootmanager by booting?\nBox will reboot after choice ')
            ybox = self.session.openWithCallback(
                self.install3, MessageBox, message, MessageBox.TYPE_YESNO)
            ybox.setTitle(_('Install Confirmation'))
        else:
            self.session.open(MessageBox, _(
                'Installation aborted !'), MessageBox.TYPE_INFO)
        ...
        if yesno:
            ...
            if fileExists('/proc/mounts'):
                ...
                for line in f.readlines():
                    if line.find(self.mysel):
                        mntdev = line.split(' ')[0]
                        f.close()
                        mntid = os.system('blkid -s UUID -o value ' + mntdev +
                                          '>/usr/lib/enigma2/python/Plugins/Extensions/OPDBoot/bin/install')
                        os.system("mv /etc/fstab /etc/fstab1")
                        os.system(
                            "grep -v  /media/opdboot /etc/fstab1 > /etc/fstab")
                        os.system("rm /etc/fstab1")
                        os.system('blkid -s UUID -o value ' + mntdev +
                                  '>/usr/lib/enigma2/python/Plugins/Extensions/OPDBoot/bin/install')
                        fstabuuid = os.popen(
                            'blkid -s UUID -o value ' + mntdev).read()
                        fstabuuidwrite = 'UUID=' + fstabuuid.strip() + \
                            '       /media/opdboot      auto        defaults           1        1'
                        ...
                        os.system(
                            'chmod 777 /sbin/opdinit;chmod 777 /sbin/init;ln -sfn /sbin/opdinit /sbin/init')
                        os.system(
                            'mv /etc/init.d/volatile-media.sh /etc/init.d/volatile-media.sh.back')
                        out3 = open('/media/opdboot/OPDBootI/.timer', 'w')
                        out3.write(
                            config.OPDBootmanager.bootmanagertimeout.value)
                        out3.close()
                        out2 = open('/media/opdboot/OPDBootI/.opdboot', 'w')
                        out2.write('Flash')
                        out2.close()
                        out = open(
                            '/usr/lib/enigma2/python/Plugins/Extensions/OPDBoot/.opdboot_location', 'w')
                        out.write(self.mysel)
                        out.close()
                        os.system(
                            'cp /usr/lib/enigma2/python/Plugins/Extensions/OPDBoot/.opdboot_location /etc/opd/')
                ...
                if 'build' not in image:
                    ...
                    for line in f.readlines():
                        if 'build=' in line:
                            ...
                            open('/media/opdboot/OPDBootI/.Flash',
                                 'w').write(image)
                            break
            ...
            if fileExists('/proc/mounts'):
                ...
                for line in f.readlines():
                    if line.find(self.mysel):
                        ...
                        os.system(cmd)
                        os.system(
                            'cp /usr/lib/enigma2/python/Plugins/Extensions/OPDBoot/bin/opdinitnoboot /sbin/opdinit')
                        os.system(
                            'chmod 777 /sbin/opdinit;chmod 777 /sbin/init;ln -sfn /sbin/opdinit /sbin/init')
                        os.system(
                            'mv /etc/init.d/volatile-media.sh /etc/init.d/volatile-media.sh.back')
            ...
            if fileExists('/etc/image-version'):
                if 'build' not in image:
                    ...
                    f.close()
            self.myclose2(
                _('OPDBoot has been installed succesfully, and Box rebooting now!'))
...
class OPDBootImageChoose(Screen):
    ...
    def __init__(self, session):
        ...
        self['label11'] = Label('')
        self['label1'] = Label(
            _('Here is the list of installed images in Your STB. Please choose an image to boot.'))
    ...
    def bootsetup(self):
        ...
        if getMachineBuild() not in ("u5", "u51", "u52", "u53", "u54", "u55", "u56", "u41", "u5pvr", "sf8008", "sf8008m"):
            ...
            menulist.append(
                (_('Setup Bootmanagertimeout'), 'bootmanagertimeout'))
        else:
            menulist.append((_('Boot without Bootmanager'), 'withoutopdboot'))
        self.session.openWithCallback(self.menuBootsetupCallback, ChoiceBox, title=_(
            'What would You like to do ?'), list=menulist)
    ...
    def menuBootsetupCallback(self, choice):
        ...
        if choice is None:
            ...
            if choice[1] == 'withopdboot':
                ...
                self.session.openWithCallback(self.close, Console, _(
                    'OPDBoot work with Bootmanager by Booting!'), [cmd0, cmd1])
            if choice[1] == 'withoutopdboot':
                cmd0 = 'cp /usr/lib/enigma2/python/Plugins/Extensions/OPDBoot/bin/opdinitnoboot /sbin/opdinit'
                cmd1 = 'chmod 777 /sbin/opdinit;chmod 777 /sbin/init;ln -sfn /sbin/opdinit /sbin/init'
                self.session.openWithCallback(self.updateList, Console, _(
                    'OPDBoot work without Bootmanager by Booting!'), [cmd0, cmd1])
            if choice[1] == 'bootmanagertimeout':
                self.session.openWithCallback(
                    self.setupDone, Setup, 'bootmanagertimeout', '/usr/lib/enigma2/python/OPENDROID')
            return
    ...
    def boot(self):
        ...
        if self.mysel:
            ...
            message = _(
                'Are you sure you want to start the image now?:\n') + self.mysel + ' ?'
            ybox = self.session.openWithCallback(
                self.boot2, MessageBox, message, MessageBox.TYPE_YESNO)
    ...
    def boot2(self, yesno):
        if yesno:
            ...
            ybox = self.session.openWithCallback(
                self.nextboot, MessageBox, message, MessageBox.TYPE_YESNO)
    ...
    def nextboot(self, yesno):
        if yesno:
            ...
            os.system('touch /tmp/.opdreboot')
            self.session.open(MessageBox, _(
                'Image will be booted on the next STB boot!'), MessageBox.TYPE_INFO)
        else:
            self.session.open(MessageBox, _(
                'Boot image not changed.'), MessageBox.TYPE_INFO)
        ...
        if self.mysel:
            ...
            try:
                if mypath == self.mysel:
                    self.session.open(MessageBox, _(
                        'Sorry you cannot delete the image, is currently booted from or select for next booting'), MessageBox.TYPE_INFO, 4)
                if self.mysel.startswith('Flash'):
                    self.session.open(MessageBox, _(
                        'Sorry you cannot delete Flash image'), MessageBox.TYPE_INFO, 4)
                else:
                    out = open('/media/opdboot/OPDBootI/.opdboot', 'w')
                    out.write('Flash')
                    out.close()
                    ...
                    ybox = self.session.openWithCallback(
                        self.remove2, MessageBox, message, MessageBox.TYPE_YESNO)
    ...
    def remove2(self, yesno):
        if yesno:
            ...
            self.session.openWithCallback(self.up, Console, _(
                'OPDBoot: Deleting Image'), [cmd, cmd1])
        else:
            self.session.open(MessageBox, _(
                'Removing canceled!'), MessageBox.TYPE_INFO)
    ...
    def installMedia(self):
        ...
        if images is True:
            ...
            mess = _(
                'The /media/opdboot/OPDBootUpload directory is EMPTY!\n\nPlease upload a zip file to install')
    ...
    def install(self):
        ...
        if count > 12:
            ...
            menulist = []
            menulist.append(
                (_('Install from /media/opdboot/OPDBootUpload'), 'media'))
            menulist.append(
                (_('Install from Internet (OpenOPD,OpenATV,OpenNFR,OpenVIX,OpenHDF)'), 'internet'))
            self.session.openWithCallback(self.menuCallback, ChoiceBox, title=_(
                'Choose they way for installation!'), list=menulist)
    ...
    def advanced(self):
        ...
        self.session.openWithCallback(self.menuAdvancedCallback, ChoiceBox, title=_(
            'What would You like to do ?'), list=menulist)
    ...
    def menuAdvancedCallback(self, choice):
        ...
        if choice is None:
            ...
            if choice[1] == 'rmallimg':
                ...
                self.session.openWithCallback(self.updateList, Console, _(
                    'OPDBoot: Deleting All Images'), [cmd, cmd1])
            return
...
class OPDBootImageInstall(Screen, ConfigListScreen):
    ...
    def createSetup(self):
        ...
        self.list.append(getConfigListEntry(
            _('Copy Settings to the new Image'), self.sett))
        self.list.append(getConfigListEntry(
            _('Boot new Image directly?'), self.bootquest))
        self.list.append(getConfigListEntry(
            _('Delete Download Imagezip after Install?'), self.zipdelete))
...
def main(session, **kwargs):
    ...
    if m == 1:
        try:
            f = open(
                '/usr/lib/enigma2/python/Plugins/Extensions/OPDBoot/.opdboot_location', 'r')
            mypath = f.readline().strip()
            f.close()
        ...
        session.open(MessageBox, _(
            'Sorry: OPDBoot no work by this Box!'), MessageBox.TYPE_INFO, 3)
    else:
        session.open(MessageBox, _(
            'Sorry: Wrong image in flash found. You have to install in flash OPD Image'), MessageBox.TYPE_INFO, 3)
