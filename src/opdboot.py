...
extensions_path_extractpy = extensions_path + \
    'OPDBoot/ubi_reader/ubi_extract_files.py'
...
def OPDBootMainEx(source, target, installsettings, bootquest, zipdelete, getimagefolder, getMachineRootFile, getImageArch):
    ...
    rc = OPDBootExtract(source, target, zipdelete,
    ...
    list_two = ['mkdir -p ' + media_opendroid_target + '/media' + dev_null,
                ...
                'cp /etc/network/interfaces ' + media_opendroid_target +
                '/etc/network/interfaces' + dev_null,
                ...
                'cp /etc/wpa_supplicant.conf ' + media_opendroid_target +
                '/etc/wpa_supplicant.conf' + dev_null,
                'rm -rf ' + media_opendroid_target + extensions_path + 'HbbTV',
                'cp -r ' + extensions_path + 'OPDBoot/OPDBoot_client ' +
                media_opendroid_target + extensions_path + dev_null,
    ...
    list_four = ['/etc/fstab', '/usr/lib/enigma2/python/Components/config.py',
                 '/usr/lib/enigma2/python/Tools/HardwareInfoVu.py']
    ...
    for file_name in os.listdir(mypath):
        ...
        list_five = ['-bootlogo.postinst', '-bootlogo.postrm',
                     '-bootlogo.preinst', '-bootlogo.prerm']
    ...
    if bootquest == "True":
        ...
        out.close()
        os.system(
            'echo "[OPDBoot] Image-Install ready, Image starts by next booting, please push exit!"')
...
def OPDBootExtract(source, target, zipdelete, getimagefolder, getMachineRootFile, getImageArch):
    ...
    if os.path.exists(sourcefile):
        ...
        if "cortexa15hf-neon-vfpv4" in getImageArch:
            ...
            print(
                '[OPDBoot] Extracting tar.bz2 image and moving extracted image to our target')
            if os.path.exists('/usr/bin/bzip2'):
                sfolder = media_nf + '/OPDBootUpload/%s' % getimagefolder
                cmd = 'tar -jxf ' + sfolder + '/rootfs.tar.bz2 -C ' + \
                    media_nf + '/OPDBootI/' + target + ' > /dev/null 2>&1'
            os.system(cmd)
            os.chdir('/home/root')
        else:
            if os.path.exists(media_nf + '/OPDBootUpload/%s' % getimagefolder):
            ...
            print(
                '[OPDBoot] Extracting UBIFS image and moving extracted image to our target')
            if os.path.exists(extensions_path_extractpyo):
                os.chmod(extensions_path_extractpyo, 0o777)
                cmd = 'python ' + extensions_path_extractpyo + \
                    ' rootfs.bin -o ' + media_nf + '/ubi'
            else:
                os.chmod(extensions_path_extractpy, 0o777)
                cmd = 'python ' + extensions_path_extractpy + \
                    ' rootfs.bin -o ' + media_nf + '/ubi'
            print(cmd)
            os.system(cmd)
            os.chdir('/home/root')
            os.system('cp -r -p ' + media_nf + '/ubi/rootfs/* ' +
                      media_nf + '/OPDBootI/' + target)
