...
class OPDChooseOnLineImage(Screen):
    ...
    def KeyOk1(self, res=None):
        ...
        if returnValue is not None:
            self.session.openWithCallback(
                self.quit, DownloadOnLineImage, returnValue, mbimageValue)
        return
    ...
    def KeyOk(self):
        ...
        if returnValue in ('opennfr', 'openhdf', 'openatv', 'pure2'):
            ...
            MBImagelist = [("6.2", _("6.2")), ("6.3", _("6.3")), ("6.4", _("6.4")), ("6.5", _(
                "6.5")), ("7.0", _("7.0")), ("7.1", _("7.1")), ("7.2", _("7.2")), ("7.3", _("7.3"))]
            ...
            if returnValue == 'opennfr':
                config.usage.mbimageversion = ConfigSelection(
                    default="6.4", choices=MBImagelist)
            else:
                config.usage.mbimageversion = ConfigSelection(
                    default="6.5", choices=MBImagelist)
            self.session.openWithCallback(self.KeyOk1, Setup, "multiboot")
            mbimageValue = config.usage.mbimageversion.value
        else:
            config.usage.mbimageversion = ConfigSelection(
                default="7.0", choices=[("0.0", _("0.0"))])
...
class DownloadOnLineImage(Screen):
    ...
    def __init__(self, session, distro, mbimageversion):
        ...
        if self.distro == 'opennfr':
            ...
            if ImageVersion == "6.5":
                ...
                hdfImageVersion = "v64"
            self.feedurl = 'https://%s.hdfreaks.cc/%s' % (
                hdfImageVersion, boxname)
    ...
    def box(self):
        ...
        if self.distro == 'opendroid':
            if box == "sf4008":
                box = "sf4008"
            self.feedurl1 = self.feedurl + "/" + BRANDOEMDROID + \
                '/index.php?open=' + MASCHINEBUILD
            req = urllib.request.Request(self.feedurl)
            reg1 = requests.get(self.feedurl1)
            stb = 'no Image for this Box on this Side'
            from bs4 import BeautifulSoup
    ...
    def green(self, ret=None):
        ...
        if sel == None:
            ...
            if self.distro == 'openvix':
                ...
                url = 'http://pur-e2.club/OU/images/' + \
                    ImageVersion + '/' + BRANDOEM + '/' + sel
            elif self.distro == 'opendroid':
            ...
            try:
                ...
                self.session.open(MessageBox, _(
                    'The URL to this image is not correct !!'), type=MessageBox.TYPE_ERROR)
                self.close()
            ...
            try:
                ...
                self.session.openWithCallback(
                    self.ImageDownloadCB, JobView, job, backgroundable=False, afterEventChangeable=False)
                return
            except Exception:
                self.session.open(MessageBox, _(
                    'The URL to this image is not correct !!'), type=MessageBox.TYPE_ERROR)
    ...
    def ImageDownloadCB(self, ret):
        if ret:
            ...
            if len(job_manager.failed_jobs) == 0:
                self.session.openWithCallback(self.startInstall, MessageBox, _(
                    'Do you want to install this image now?'), default=False)
            else:
                self.session.open(MessageBox, _(
                    'Download Failed !!'), type=MessageBox.TYPE_ERROR)
            return
                ...
                if self.feed == "openeight":
                    ...
                    if self.feed in 'openatv':
                        self.imagelist.append(
                            line[t + tt + 10:t + tt + tt + 39].decode())
                    ...
                    self.imagelist.append(
                        line[t4 + len(box1) + 6:t5 + 4].decode())
...
class ImageDownloadTask(Task):
    ...
    def run(self, callback):
        ...
        self.download.start().addCallback(
            self.download_finished).addErrback(self.download_failed)
    ...
    def download_progress(self, recvbytes, totalbytes):
        if recvbytes - self.last_recvbytes > 10000:
            ...
            self.name = _('Downloading') + ' ' + \
                '%d of %d kBytes' % (recvbytes / 1024, totalbytes / 1024)
