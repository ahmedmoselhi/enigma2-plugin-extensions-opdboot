...
class common_hdr(object):
    ...
    def __init__(self, buf):
        fields = dict(zip(UBIFS_COMMON_HDR_FIELDS,
...
class sb_node(object):
    ...
    def __init__(self, buf):
        fields = dict(zip(UBIFS_SB_NODE_FIELDS,
...
class mst_node(object):
    ...
    def __init__(self, buf):
        fields = dict(zip(UBIFS_MST_NODE_FIELDS,
...
class dent_node(object):
    ...
    def __init__(self, buf):
        fields = dict(zip(UBIFS_DENT_NODE_FIELDS,
                      struct.unpack(UBIFS_DENT_NODE_FORMAT, buf)))
        for key in fields:
            if key == 'key':
...
class data_node(object):
    ...
    def __init__(self, buf):
        fields = dict(zip(UBIFS_DATA_NODE_FIELDS,
                      struct.unpack(UBIFS_DATA_NODE_FORMAT, buf)))
        for key in fields:
            if key == 'key':
                setattr(self, key, parse_key(fields[key]))
            else:
...
class idx_node(object):
    ...
    def __init__(self, buf):
        fields = dict(zip(UBIFS_IDX_NODE_FIELDS,
                      struct.unpack(UBIFS_IDX_NODE_FORMAT, buf)))
        for key in fields:
            setattr(self, key, fields[key])
...
class ino_node(object):
    ...
    def __init__(self, buf):
        fields = dict(zip(UBIFS_INO_NODE_FIELDS,
                      struct.unpack(UBIFS_INO_NODE_FORMAT, buf)))
        for key in fields:
            if key == 'key':
                setattr(self, key, parse_key(fields[key]))
            else:
...
class branch(object):
    ...
    def __init__(self, buf):
        fields = dict(
            zip(UBIFS_BRANCH_FIELDS, struct.unpack(UBIFS_BRANCH_FORMAT, buf)))
        for key in fields:
            setattr(self, key, fields[key])
